DELETE FROM users;
insert into users values(1,'Atika','atika@gmail.com','atika');
insert into users values(2,'Siddhant','siddhant@gmail.com','siddhant');
insert into users values(3,'Akshatha','akshatha@gmail.com','akshatha');
insert into users values(4,'Soumili','soumili@gmail.com','soumili');
insert into users values(5,'Abhijeet','abhijeet@gmail.com','abhijeet');
insert into users values(6,'Rohan','rohan@gmail.com','rohan');
